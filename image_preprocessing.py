...
def main():
    ...
    parser.add_argument('--functions', type=str, help='An optional argument to specify which image preprocessing functions to run (comma-seperated). If not specified, all functions will be run. Ex: "-- functions amine_rhone,clahe,gc"')
    ...
    functions = []
    if functionsStr :
        for function in functionsStr.split(","):
            try:
                functions.append(possibles.get(function))
            except Exception:
                print(function + " is not a function in the ImagePreprocessing class")
                return
    else:
