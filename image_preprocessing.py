...
def main():
    ...
    parser.add_argument('--image', type=str, help='An optional argument to specify which image in the test_images folder should be used. If not specified, frame216.jpg will be used.')
    ...
    # If the image argument was specified, check if the image exists and replace the value of imgPath with it
    # If the image does not exist, let the user know via command line and stop execution
    # If the image argument was not specified, imgPath contains frame216.jpg as the default
    imgPath = "test_images/frame216.jpg"
    if args.image:
        if exists("test_images/" + args.image):
            imgPath = "test_images/" + args.image
        else:
            print("There is no file with name " + args.image + " in the test_images directory.")
            return
